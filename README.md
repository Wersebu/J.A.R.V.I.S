...
Current version: **`2.18.4`**. Runs on Java 21 with Maven, targets Ubuntu Server 24.04 LTS or Windows, and talks to a local Ollama instance for inference.
...
**Deterministic selection, never an extra LLM call.** The main model's first decision happens before it has analyzed anything, so it can never be trusted to correctly ask for an image it has not seen yet. `ConversationImageResolver` (`jarvis-memory.image`) decides purely from the current message's own text, using plain token matching (diacritic/case-normalized, no external NLP): an explicit `image-N` label, a mentioned file name, an ordinal reference ("drugie zdjęcie", "the last screenshot"), optionally qualified by which earlier message it came from ("drugie zdjęcie z pierwszej wiadomości" - a message can name *several* distinct images at once, e.g. "compare X with Y", each resolved independently), or - only when nothing more specific matches and `auto-attach-mode: REFERENCED_OR_RECENT` - a vague reference ("tamto zdjęcie", "screen wyżej") falls back to the most recent available historical images up to the configured limits. A message with **no** image-related word at all never auto-attaches anything from history. Every current-message image is always included regardless. Duplicates between current and historical selections are removed; the exact-limit-hit case reports which historical matches were left out (`skippedDueToLimit`) so the model can be told honestly instead of silently truncating.
...
**`=== CONVERSATION IMAGES ===` prompt block**, appended by `PromptBuilderStage` right after the base prompt (so both the main decision call and the tool loop's base prompt see it), lists every available/expired historical image with its source message, file name, status, expiry, and whether it was actually passed as native input this turn - and instructs the model never to claim an `AVAILABLE`+passed image is inaccessible, and never to claim it can still see an `EXPIRED`/`MISSING` one. This text block is metadata only; the actual pixels always travel separately through `messages[].images`.
...
**`ToolCallingStage` now reports the real reason directly instead of ever attempting tool-less synthesis for it.** When `finalAnswer` is blank and the termination reason is one of the "loop genuinely did not reach a verified answer" set (`MAX_TURNS_REACHED`, `TIMEOUT`, `MAX_FAILURES_REACHED`, `MAX_OPERATION_REPEATS_REACHED`, `EMPTY_MODEL_RESPONSE`, `PROVIDER_FAILURE`, `MCP_FAILURE`, `INCOMPLETE_GOAL`), `streamToolFinalAnswer` builds a structured plain-text report (`buildTerminationReport`) straight from `ToolLoopTerminationInfo` and returns it - the tool-less synthesis call (and therefore its own chance to emit a stray `TOOL_REQUEST`) is skipped entirely for this case. `COMPLETED` and `WAITING_FOR_APPROVAL` are unaffected and keep the existing behavior.
