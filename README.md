...
jarvis:
  ...
  tools:                    # native tool-calling runtime budgets (see Native Tool Calling)
    ...
    timeout-seconds: 0        # 0 disables the wall-clock timeout (bounded by max-calls-* instead)
...
- **Wall-clock timeout (opt-in)**: `timeout-seconds`, when set above 0, bounds the whole loop regardless of call count. Default is `0` (disabled) - the loop is bounded by `max-calls-*` and the no-progress/duplicate-detection guards instead, so a model still making real progress is never cut off purely because a clock ran out.
