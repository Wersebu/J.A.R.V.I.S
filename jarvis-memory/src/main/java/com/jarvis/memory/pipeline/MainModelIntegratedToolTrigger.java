...
public class MainModelIntegratedToolTrigger implements ToolTriggerStrategy {
    ...
    public String buildMainModelPrompt(PipelineContext context) {
        ...
        return context.prompt()
                ...
                + "- Keep \"goal\" and \"reason\" short - one or two sentences describing WHAT needs doing and WHY, "
                + "never the actual content. If the task is to create/update a document, write code, or produce any "
                + "long text, do NOT put that text in this JSON at all - the concrete tool call that actually writes "
                + "it happens in a separate later step, where that content is a normal tool-call argument, not part "
                + "of this envelope. Cramming long content into \"goal\"/\"reason\" here often breaks JSON escaping "
                + "and wastes an entire turn.\n\n"
                + "Schemas:\n"
